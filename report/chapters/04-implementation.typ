...
+ 调用数据库存储过程`receive_parcel()`，该过程内部完成:
...
*设计优势*：运费计算逻辑统一封装在数据库中，避免应用层重复实现，且便于后续调整价格策略。
...
*设计说明*：通过视图简化查询逻辑，定时任务只需关注业务流程，无需关心复杂的SQL计算。
...
系统采用基于角色的访问控制(RBAC)模型，定义了三种用户角色。普通用户(user)可查询自己的包裹和创建寄件单；驿站员工(staff)可进行包裹入库、取件操作以及查询所有包裹；管理员(admin)可进行用户管理、系统配置和数据导出。
...
系统采用多层次的数据安全保护措施。密码使用bcrypt算法加密存储，有效防止明文泄露。敏感信息如身份证号通过视图脱敏展示。SQL注入防护通过使用GORM参数化查询和存储过程实现。XSS防护通过前端对用户输入进行转义。生产环境启用HTTPS传输，防止中间人攻击。
